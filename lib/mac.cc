...
mac_impl(bool debug, int fcf, int seq_nr, int dst_pan, int dst, int src, float thold) :
	block ("mac",
...

{
	...
	for(size_t i = 0; i < noutput_items; i++) {
		if((in[i] > d_thold) && (in[i] < -0.01)){
			sum_rssi += in[i];
			count++;
			/*if(count > 50000){
				dout << (sum_rssi/count) << std::endl;
				sum_rssi = 0;
				count = 0;
			}*/
		}
...
mac::sptr
mac::make(bool debug, int fcf, int seq_nr, int dst_pan, int dst, int src, float thold) {
	return gnuradio::get_initial_sptr(new mac_impl(debug,fcf,seq_nr,dst_pan,dst,src, thold));
}
